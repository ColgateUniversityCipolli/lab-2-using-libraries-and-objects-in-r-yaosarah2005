...
I used the `esquisse` [@esquisse] package to draw the graph
...
Adelie has relative low body mass on average, but relative high count. Chinstrp has relative similar body mass to Adelie, but has low count. Gentoo has relative high body mass on average and relative low count.
...
The resulting class is "character", because R converts elements into more general class when creating the vector, following the order "logical → integer → numeric → complex → character", and since "MATH" is in the class of "character", so 240 is coerced to a character.